...
Dans le backoffice, clique le bouton **🔑 Clé OpenAI** (en haut à droite),
colle ta clé, puis **Enregistrer**. C'est tout — rien à taper dans un terminal
ni à éditer à la main. Elle est partagée par toutes les boutiques et enregistrée
dans un fichier `.env` local.

> Le bouton clignote en orange (⚠️) tant qu'aucune clé n'est enregistrée, et
> affiche ✓ une fois que c'est fait.
