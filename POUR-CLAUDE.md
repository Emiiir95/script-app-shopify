...
> 💡 L'utilisateur peut aussi la saisir **lui-même sans toi** : dans le backoffice,
> bouton **🔑 Clé OpenAI** (en haut à droite) → coller → Enregistrer. Le backoffice
> l'écrit dans `.env` (endpoint `POST /api/openai-key`, refuse une clé qui ne
> commence pas par `sk-`).
